...
function deObjetoAmatriz(objeto){
  ...
  for(var key in objeto){
    array.push([key,objeto[key]])// push (2 elementos: key , value)
  } return array
}
...
function numberOfCharacters(string) {
  ...
  for(var i=0;i<string.length;i++) {
    if(!objeto[string[i]]){  // xq 
      objeto[string[i]]=1
    }else {objeto[string[i]]++}
  }
  return objeto; 
}
...
function asAmirror(str) {
  ...
  // let arrayfinal=stringArray.map(function(element)  {
